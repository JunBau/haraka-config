...
// `owner` is the Config instance, one per plugin. Callers that omit it are
// indistinguishable, so they share one slot, as they did before registrations
// existed.
function same_reader(a, b) {
  if (a.owner !== b.owner) return false
  if (a.owner === undefined) return true
  return a.type === b.type && a.options_key === b.options_key
}

// Callbacks re-read and pass a fresh closure, so a reader that reads again
// replaces its entry instead of adding one.
function register(readers, entry) {
  for (const existing of readers) {
    if (!same_reader(existing, entry)) continue
...
class Reader {
  ...
  get_cache_key(name, type, options) {
    // An injected `!file` override is stored under the bare path, with no type
    // or options to key on, so an overridden file keeps the untyped slot.
    if (this._overrides[name]) return name

    // The parsed shape depends on the type, so two types can't share a slot:
    // reloading one reader of a file would hand the other the wrong shape.
    // No options means the no-options slot. Inheriting the options of whoever
    // read this file last would hand a bare reader another reader's slot.
    // ordering of objects isn't guaranteed to be consistent, but typically is.
    const key = [type ?? types.type_of(name)]
    if (options) key.push(options)
    return name + JSON.stringify(key)
  }

  read_config(name, type, cb, options, owner) {
    ...
    // 3. reload every caller watching this file

    const previous = this._read_args[name]
    const previous_key = previous && this.get_cache_key(name, previous.type, previous.options)
    const readers = previous?.readers ?? []
    this._read_args[name] = { type, cb, options, readers }
    const cache_key = this.get_cache_key(name, type, options)
    // get_cache_key() collapses an overridden file to bare `name`, so identity
    // tracks the requested options separately.
    const options_key = options ? JSON.stringify(options) : ''
    register(readers, { type, cb, options, owner, options_key })
    ...
    try {
      switch (type) {
        ...
        case 'yaml':
          ...
          this.process_file_overrides(name, type, options, result, cache_key)
          break
        // case 'binary':
        default:
  ...
  last_load_error(name, type, options) {
    ...
    return this._load_errors[this.get_cache_key(name, type, options)]
  }
