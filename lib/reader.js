...
// Callbacks re-read and pass a fresh closure, so entries are keyed by
// (key, owner). `owner` is the Config instance, one per plugin.
function register(readers, key, type, cb, options, owner) {
  for (const reader of readers) {
    if (reader.key !== key || reader.owner !== owner) continue
    reader.type = type
    reader.cb = cb
    reader.options = options
    return
  }
  readers.push({ key, type, cb, options, owner })
}
...
class Reader {
  ...
  read_config(name, type, cb, options, owner) {
    ...
    // cache_key already encodes options, so reusing it skips a second stringify.
    register(readers, `${type}\u0000${cache_key}`, type, cb, options, owner)
