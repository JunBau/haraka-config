...
Watch.reload = (reader, name, args) => {
  ...
  const readers = args.readers?.length ? [...args.readers] : [args]
  let first_err

  for (const reader_args of readers) {
    // Cache entries are keyed by name+options, so each reader needs its own.
    ...
    try {
      ...
      console.error(`Reload callback for ${name} threw: ${e.message}`)
    }
  }
  ...
  enoent.timer = setInterval(() => {
    ...
    for (const file of [...enoent.files]) {
      fs.stat(file, (err) => {
        ...
        // the caller tore this slot down while we were polling
        if (!args) return
...
Watch.file = (reader, name, type, cb, options) => {
  ...
  if (watchers[name] || options?.no_watch) return
...
Watch.dir = (reader, dir_path) => {
  ...
  try {
    watchers[cp] = fs.watch(cp, { persistent: false }, (fse, filename) => {
      ...
      if (!args) return
      // getDir() registers the directory path itself in _read_args (as { opts });
      // its own recursive watcher (watch.dir2) handles reloads. Reloading it here
      // would call load_config() on a directory and throw EISDIR.
      if (args.opts) return
      ...
      sedation_timers[full_path] = setTimeout(() => {
        ...
        if (latest && !latest.opts) Watch.reload(reader, full_path, latest)
      }, 5 * 1000)
    })
    watchers[cp].unref?.()
  } catch (e) {
...
Watch.close = (reader, target) => {
  ...
  // the poller would otherwise re-attach a watcher the caller just dropped
...
Watch.onEvent = (reader, name, args) => {
  return (fse) => {
    ...
    sedation_timers[name] = setTimeout(() => {
      ...
      Watch.reload(reader, name, latest())
    }, 5 * 1000)
